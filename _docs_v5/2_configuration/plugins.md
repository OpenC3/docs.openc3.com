...
This document provides the information necessary to configure a OpenC3 plugin. Plugins are how you configure and extend OpenC3 5.  

Plugins are where you define targets (and their corresponding command and telemetry packet definitions), where you configure the interfaces needed to talk to targets, where you can define routers to stream raw data out of OpenC3, how you can add new tools to the OpenC3 user interface, and how you can run additional microservices to provide new functionality.
...
Targets are the external pieces of hardware and/or software that OpenC3 communicates with.  These are things like Front End Processors (FEPs), ground support equipment (GSE), custom software tools, and pieces of hardware like satellites themselves.  A target is anything that OpenC3 can send commands to and receive telemetry from.
...
Routers flow streams of telemetry packets out of OpenC3 and receive streams of commands into OpenC3.  The commands are forwarded by OpenC3 to associated interfaces.  Telemetry comes from associated interfaces.

### Tool

OpenC3 Tools are web-based applications the communicate with the OpenC3 APIs to perform takes like displaying telemetry, sending commands, and running scripts.

### Microservice

Microservices are persistent running backend code that runs within the OpenC3 environment.  They can process data and perform other useful tasks.

## Plugin Directory Structure

OpenC3 plugins have a well-defined directory structure as follows:
...
| LICENSE.txt | License for the plugin.  OpenC3 Plugins should be licensed in a manner compatible with the AGPLv3, unless they are designed only for use with OpenC3 Enterprise Edition (or only for use at Ball), in which case they can take on any license as desired. |
| openc3-PLUGINNAME.gemspec | This file should have the name of the plugin with a .gemspec extension.  For example openc3-demo.gemspec.   The name of this file is used in compiling the plugin contents into the final corresponding <Plugin Name>-<Version>.gem plugin file.  ie. openc3-demo-5.0.0.gem.  OpenC3 plugins should always begin with the openc3- prefix to make them easily identifiable in the Rubygems repository. The contents of this file take on the Rubygems spec format as documented at: https://guides.rubygems.org/specification-reference/ |
| Rakefile | This is a helper ruby Rakefile used to build the plugin.  In general, the exact same file as provided with the OpenC3 demo plugins can be used, or other features can be added as necessary for your plugin project.  This file is typically just used to support building the plugin by running "rake build VERSION=X.X.X" where X.X.X is the plugin version number.  A working Ruby installation is required.  |
...
| targets/TARGETNAME/cmd_tlm | This folder contains the command and telemetry definition files for the target. These files capture the format of the commands that can be sent to the target, and the telemetry packets that are expected to be received by OpenC3 from the target. Note that the files in this folder are processed in alphabetical order by default. That can matter if you reference a packet in another file (it should already have been defined). See the Command and Telemetry Configuration Guides for more information. |
...
A plugin.txt configuration file is required for any OpenC3 plugin.   It declares the contents of the plugin and provides variables that allow the plugin to be configured at the time it is initially installed or upgraded.
This file follows the standard OpenC3 configuration file format of keywords followed by zero or more space separated parameters.  The following keywords are supported by the plugin.txt config file:
