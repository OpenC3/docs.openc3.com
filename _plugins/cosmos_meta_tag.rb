...
module Jekyll
  ...
  class CosmosMetaTag < Liquid::Tag
    ...
    def render(context)
      ...
      root = File.expand_path(context.registers[:site].config["openc3_root_#{version}"]).strip
      path = File.join(root, context.registers[:site].config['openc3_meta_path'].strip)
